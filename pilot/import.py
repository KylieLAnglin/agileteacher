...
def create_participant_df(df: pd.DataFrame, name:str):
    ...
    attempts = []
    texts = []
    for attempt in ['First Directions Elements',
    'Second Directions Elements', 'Third Directions Elements']:
        text = ''
...
df = pd.read_excel(os.path.join(start.raw_data_path, "pilot_study_data.xlsx"),
sheet_name="Esteban - lower codes and time", skiprows=5)
text_df = create_participant_df(df = df, name='esteban')

participant_list = [("Jonathan - lower codes and time", 'jonathan'),
...
for participant in participant_list:
    df = df = pd.read_excel(os.path.join(start.raw_data_path, "pilot_study_data.xlsx"),
        sheet_name=participant[0], skiprows=5)
